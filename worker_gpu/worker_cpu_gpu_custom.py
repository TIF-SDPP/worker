...
def keep_alive():
    url = "http://35.227.11.190:8080/keep_alive"
...
def post_result(data):
    url = "http://35.196.188.172:8080/solved_task"
    try:
        response = requests.post(url, json=data)
        print("Post response:", response.text)
    except requests.exceptions.RequestException as e:
...
def main():
    connection = pika.BlockingConnection(
        pika.ConnectionParameters(host='104.196.212.234', port=5672, credentials=pika.PlainCredentials('guest', 'guest'))
    )
    channel = connection.channel()
